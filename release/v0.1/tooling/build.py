...
DISCLAIMER_PL = "Niezależny jawnoźródłowy projekt analityczny — bez materiałów niejawnych — nie jest dokumentem rządowym."
DISCLAIMER_EN = "Independent public-source analytical draft — no classified material — not a government document."
...
.governance-law table,.implementation-roadmap table,.ministerial-briefs table { table-layout:fixed; font-size:.72rem; }
.governance-law th,.governance-law td,.implementation-roadmap th,.implementation-roadmap td,.ministerial-briefs th,.ministerial-briefs td { padding:5px 6px; }
.evidence-book .registry-table { table-layout:fixed; font-size:.70rem; }
.evidence-book .registry-table th,.evidence-book .registry-table td { padding:5px 6px; overflow-wrap:anywhere; word-break:break-word; }
.evidence-book .registry-sources th:nth-child(1),.evidence-book .registry-sources td:nth-child(1) { width:8%; }
.evidence-book .registry-sources th:nth-child(2),.evidence-book .registry-sources td:nth-child(2) { width:24%; }
.evidence-book .registry-sources th:nth-child(3),.evidence-book .registry-sources td:nth-child(3) { width:14%; }
.evidence-book .registry-sources th:nth-child(4),.evidence-book .registry-sources td:nth-child(4) { width:9%; }
.evidence-book .registry-sources th:nth-child(5),.evidence-book .registry-sources td:nth-child(5) { width:9%; }
.evidence-book .registry-sources th:nth-child(6),.evidence-book .registry-sources td:nth-child(6) { width:10%; }
.evidence-book .registry-sources th:nth-child(7),.evidence-book .registry-sources td:nth-child(7) { width:9%; }
.evidence-book .registry-sources th:nth-child(8),.evidence-book .registry-sources td:nth-child(8) { width:17%; }
.evidence-book .source-url { display:block; margin-top:3px; color:#5d6873; font-size:.58rem; line-height:1.2; }
.evidence-book .registry-claims th:nth-child(1),.evidence-book .registry-claims td:nth-child(1) { width:11%; }
.evidence-book .registry-claims th:nth-child(2),.evidence-book .registry-claims td:nth-child(2) { width:9%; }
.evidence-book .registry-claims th:nth-child(3),.evidence-book .registry-claims td:nth-child(3) { width:10%; }
.evidence-book .registry-claims th:nth-child(4),.evidence-book .registry-claims td:nth-child(4) { width:10%; }
.evidence-book .registry-claims th:nth-child(5),.evidence-book .registry-claims td:nth-child(5) { width:46%; }
.evidence-book .registry-claims th:nth-child(6),.evidence-book .registry-claims td:nth-child(6) { width:14%; }
.strategy .cover-visual { display:block; width:100%; height:88mm; object-fit:cover; object-position:center 63%; margin:7mm auto 0; break-after:page; }
...
.metadata { margin-top:6px; color:var(--muted); font-size:.72rem; line-height:1.15; padding-top:0; break-inside:auto; }
...
def evidence_markdown() -> str:
    sections = []
    configs = [
        ("Źródła", "research/sources.yaml", "sources", ["id", "title", "publisher", "published_at", "accessed_at", "tier", "status", "locator"]),
        ...
        ("Definicje kontrolowane", "research/definitions.yaml", "definitions", ["id", "term", "definition", "scope", "status"]),
    ...
    for title, relative_path, key, columns in configs:
        ...
        if not records:
            ...
            continue
        sections.append(f'<table class="registry-table registry-{key}"><thead><tr>')
        sections.extend(f"<th>{html_lib.escape(column)}</th>" for column in columns)
        ...
        for record in records:
            ...
            for column in columns:
                ...
                escaped = html_lib.escape(str(value).replace("\n", " "))
                if key == "sources" and column == "title" and record.get("url"):
                    url = html_lib.escape(str(record["url"]), quote=True)
                    escaped = (
                        f'<a href="{url}">{escaped}</a>'
                        f'<span class="source-url">{url}</span>'
                    )
                values.append(escaped)
            sections.append("<tr>" + "".join(f"<td>{value}</td>" for value in values) + "</tr>\n")
...
<body><main class="{artifact['id']}"><div class="draft-banner"><strong>{disclaimer}</strong><br>Stan wiedzy / evidence cut-off: {TODAY}</div>{body}<div class="metadata">Identyfikator: {artifact['id']} · status: {artifact['status']} · wygenerowano: {TODAY}</div></main></body></html>"""
    ...
    for element in soup.body.find_all(["h1", "h2", "h3", "h4", "p", "ul", "ol", "table", "blockquote", "img"], recursive=True):
        ...
        if element.name in {"h1", "h2", "h3", "h4"}:
            if (
                ...
                and element.get_text(" ", strip=True) == title
            ):
            ...
            if src:
                ...
                if path.exists() and ROOT in path.parents:
                    ...
                    picture = document.add_picture(str(picture_path), width=Inches(6.2))
                    alt_text = (element.get("alt") or "").strip() or f"Ilustracja: {title}"
                    picture_properties = picture._inline.docPr
                    picture_properties.set("descr", alt_text)
                    picture_properties.set("title", alt_text)
    ...
    for slide_index, slide_text in enumerate(split_slides(text), start=1):
        ...
        if image_match:
            ...
            if image_path.exists() and ROOT in image_path.parents:
                ...
                alt_text = image_match.group(1).strip() or (
                    "Concept diagram supporting this slide"
                    if artifact["language"] == "en"
                    else "Diagram koncepcyjny wspierający treść slajdu"
                )
                picture_properties = picture._element.nvPicPr.cNvPr
                picture_properties.set("descr", alt_text)
                picture_properties.set("title", alt_text)
...
def build_artifact(artifact: dict) -> dict:
    ...
    portable_html = html_text.replace('src="assets/', 'src="../assets/')
    built = []

    if "html" in artifact["outputs"]:
        target = output_dir / "index.html"
        target.write_text(portable_html, encoding="utf-8")
        built.append(str(target.relative_to(ROOT)))
    if "pdf" in artifact["outputs"]:
        target = output_dir / f"{artifact['id']}.pdf"
        HTML(string=portable_html, base_url=str(output_dir)).write_pdf(
            target,
