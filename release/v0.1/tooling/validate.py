...
def main() -> int:
    ...
    definitions = load_yaml(ROOT / "research" / "definitions.yaml").get("definitions", [])
    ...
    validate_records("definitions", definitions, "definition.schema.json", errors)
    validate_records("artifacts", artifacts, "artifact.schema.json", errors)

    source_ids = {source["id"] for source in sources}
    claim_ids = {claim["id"] for claim in claims}
    claim_status = {claim["id"]: claim.get("status") for claim in claims}
    ...
    for decision in decisions:
        ...
        non_verified_evidence = [
            ...
            if claim_status.get(claim_id) != "verified"
        ]
        if non_verified_evidence:
            errors.append(
                f"{decision['id']}: decision evidence is not verified "
                f"{sorted(non_verified_evidence)}"
            )
    ...
    print(
        f"Validated {len(sources)} sources, {len(claims)} claims, "
        f"{len(decisions)} decisions, {len(definitions)} definitions and "
